...
## Behavior
- Set capture profile to `light` (`browser.set_capture_profile`) unless user asks for high detail.
- Use DOM-first capture ladder:
  1) `browser.snapshot` (`detail: "low"`)
  2) `browser.list` (`detail: "low"`)
  3) `browser.query_dom` (`detail: "low"`)
  4) `browser.take_snapshot` (`detail: "low"`) only when `uid` targeting is needed
...
- For long pages, use `browser.get_scroll_state` and `browser.scroll_by`; use `browser.visual_snapshot` only if DOM tools remain ambiguous.
